...
const Chart = ({data, modalOpen, setModalOpen}) => {
    const [pieData, setPieData] = useState([]);

    useEffect(() => {
        const newArray = data.filter(item => item.all > 0).map(item => ({
            id: item.label,
            value: item.all,
            color: item.color,
            selected: true
    ...
    return (
        <>
            <div style={{width: '100%', height: '320px'}}>
                <ResponsivePie
                    data={pieData.filter(({selected}) => !!selected)}
                    ...
                    innerRadius={0.52}
                    colors={pieData.map(({color}) => color)}
                    borderColor={{
                        from: 'color',
                        modifiers: [
                            [
            ...
            {modalOpen ?
                <Modal title="Діаграма" closeModal={setModalOpen}>
                    <ul className={`tasks__modal-list ${data.length > 7 ? 'scroll' : ''}`}>
                        {
                            pieData.map(({id, selected}, index) =>
                                <li key={index} className="tasks__modal-item">
                                    <button
                                        className="tasks__modal-btn"
                                        onClick={() => setPieData(prevData => {
                                            const updatedData = [...prevData]; // Копіюємо масив
                                            updatedData[index] = {
                                                ...updatedData[index],
